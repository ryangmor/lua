...
def credits(m):
	cid = m.chat.id
	bot.send_message(cid, "@Ryangmorr", parse_mode="Markdown")

@bot.message_handler(commands=['help'])
def help(m):
	cid = m.chat.id
	bot.send_message(cid, "its on test now", parse_mode="Markdown")
...
def (m):
    ...
    str = m.text
    txt = str.replace("", "")
    bot.send_message(idA, "<b>Ersal Shod Gl </b>".format(txt,idA), parse_mode="HTML")
				...
				with open('user.txt', 'a') as f:
					f.write(str(id)+"\n")
		                print "New user received."
	                        bot.send_message(id, "a test on progress", parse_mode="Markdown")
        return
