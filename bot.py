...
API_TOKEN = '128124618:AAFA_V5A5xyJ_09HNFIH_SFRenRhTI2JIyM'
...
def on_user_joins(m):
	...
	if m.content_type == 'new_chat_participant':
		if m.new_chat_participant.id == bot.get_me().id:
			...
			groupid = m.chat.id
			bot.send_message('52038630', "# DEBUG # " + "Bot got invited to the group " + str(groupname) + "(" + str(groupid) + ")", parse_mode="HTML")
...
def credits(m):
	cid = m.chat.id
	bot.send_message(cid, "@rrrrr", parse_mode="Markdown")
